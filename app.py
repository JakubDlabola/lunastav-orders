...
function updatePopisDila() {{
  ...
  if (!hasRoof && !hasCeil && !hasSikminy && !hasWin) return;
...
function checkSubmit() {{
  ...
  if (!hasRoof && !hasCeil && !hasSikminy && !hasWin) {{
    missing.push('Vyberte alespoň jednu kategorii (střecha, strop, šikminy, okna)');
  }} else {{
...
def _order_form_post_inner(
    ...
    total = eligible_roof + eligible_ceiling + eligible_sikminy + eligible_win_a + eligible_win_b + eligible_win_c + blinds_cost + nets_cost + doprava_price + pochozi_total_incl
